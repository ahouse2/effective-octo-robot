  ...
  try {
    ...
    if (ai_model === 'openai') {
      ...
      if (command === 'user_prompt') {
        ...
        // Add search results to the OpenAI thread
        await openai.beta.threads.messages.create(
          openai_thread_id,
          {
            role: "user",
        ...
        while (runStatus === 'queued' || runStatus === 'in_progress' || runStatus === 'cancelling') {
          ...
          runStatus = retrievedRun.status;
          console.log(`OpenAI Run Status (Web Search): ${runStatus}`);
        }
        ...
        if (runStatus === 'completed') {
          ...
          await supabaseClient.from('agent_activities').insert({
            ...
            content: `OpenAI Assistant run for web search failed or ended with status: ${runStatus}`,
            status: 'error',
          });
          throw new Error(`OpenAI Assistant run for web search failed with status: ${runStatus}`);
        }
      } else {
