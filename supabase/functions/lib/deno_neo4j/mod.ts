...
export class Neo4j {
  ...
  private resolvers = new Map<number, { resolve: (value: any) => void; reject: (reason?: any) => void; records: any[] }>();
  ...
  private async sendHandshake(): Promise<void> {
    ...
    this.send(handshake);
    await this.waitForResponse(this.messageId); // Wait for the server to acknowledge handshake
  }
  ...
  private async sendHello(): Promise<void> {
    ...
    await this.waitForResponse(this.messageId); // Wait for the server to acknowledge hello
  }

  async query(cypher: string, params: Record<string, any> = {}): Promise<{ records: any[], summary: any }> {
    ...
    return new Promise((resolve, reject) => {
      this.resolvers.set(currentMessageId, { resolve, reject, records: [] });
    });
  }

  private async waitForResponse(id: number): Promise<any> {
    return new Promise((resolve, reject) => {
      this.resolvers.set(id, { resolve, reject, records: [] });
  ...
  private handleMessage(data: ArrayBuffer): void {
    ...
    // Find the resolver for the current message sequence.
    // This simple logic assumes responses are strictly sequential.
    // For a robust client, a more sophisticated message ID tracking is needed.
    const currentResolverId = this.messageId - 1; 
    const resolver = this.resolvers.get(currentResolverId);

    if (!resolver) {
      console.warn("No resolver found for message ID:", currentResolverId, "Signature:", signature.toString(16));
      return;
    }

    switch (signature) {
      case 0x70: // SUCCESS
        resolver.resolve({ records: resolver.records, summary: fields[0] });
        this.resolvers.delete(currentResolverId);
        break;
      case 0x7E: // FAILURE
      ...
      case 0x7F: // IGNORED
        ...
        break;
      case 0x7D: // RECORD
        resolver.records.push(fields[0]);
        break;
      default:
        console.warn("Unknown message signature:", signature.toString(16));
        break;
    }
  }
...
class Unpacker {
  ...
  unpackStructSignature(): number {
    ...
    if ((marker & 0xF0) !== 0xB0) {
      // This might be a record marker (0x7D) followed by a struct.
      // If it's a record, the actual struct signature is the first field.
      // For simplicity, we'll assume the first byte is always the struct marker for now.
      // A more robust unpacker would handle different message types.
      // For now, if it's not a struct marker, we'll just return the marker itself
      // and let the caller decide how to interpret it, or throw if unexpected.
      // Given the context of Bolt protocol, it should be a struct marker.
      // If it's not, it's likely an error in the stream or an unexpected message type.
      // Let's re-read the marker if it's not a struct, and try to unpack it as a value.
      this.offset--; // Rewind
      return this.unpack(); // Try to unpack as a value
    }
