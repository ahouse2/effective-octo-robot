...
serve(async (req) => {
  ...
  try {
    const { caseId, fileNames, caseGoals, systemInstruction } = await req.json();
    ...
    const { error: activityError } = await supabaseClient
      ...
      .insert({
        ...
        content: `Analysis initiated for case ${caseId}. Files received: ${fileNames.join(', ')}. Case Goals: ${caseGoals || 'Not specified'}. System Instruction: ${systemInstruction || 'None provided'}.`,
        status: 'processing',
      });
    ...
    // 3. Upload files to OpenAI and collect file_ids
