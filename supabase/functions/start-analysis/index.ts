...
serve(async (req) => {
  ...
  try {
    ...
    if (aiModel === 'openai') {
      ...
      let assistant = null;
      assistantId = Deno.env.get('OPENAI_ASSISTANT_ID'); // Try to get existing ID from env
      ...
      if (!assistantId) {
        try {
          assistant = await openai.beta.assistants.create({
            ...
            instructions: `You are a specialized AI assistant for California family law cases. Your primary goal is to analyze evidence, identify key facts, legal arguments, and potential outcomes. Always cite the source document when making claims.
            
            User's Case Goals: ${caseGoals || 'Not specified.'}
            User's System Instruction: ${systemInstruction || 'None provided.'}
            
            You have access to tools to help you with your analysis:
            - Use the 'search_files' tool to search within the uploaded case files for specific information.
            - Use the 'web_search' tool to perform a web search for external legal information or general knowledge.
            ...
            tools: [
              { type: "file_search" }, // Built-in file search
              {
                ...
                function: {
                  name: "search_files",
                  description: "Search for specific information within the uploaded case files.",
                  parameters: {
                    type: "object",
                    properties: {
                      query: {
                        type: "string",
                        description: "The search query to find information within the case files.",
                      },
                    },
                    required: ["query"],
                  },
              ...
              },
              {
                ...
                function: {
                  name: "web_search",
                  description: "Perform a web search for external legal information or general knowledge.",
                  parameters: {
                    type: "object",
                    properties: {
                      query: {
        ...
        openaiAssistantId = assistantId; // Use the retrieved assistant ID
      }
      ...
      await supabaseClient.from('agent_activities').insert({
        ...
        content: `OpenAI Assistant (ID: ${openaiAssistantId}) started analysis on Thread (ID: ${openaiThreadId}). Run ID: ${run.id}.`,
        status: 'processing',
      });

    } else if (aiModel === 'gemini') {
      // For Gemini, initial setup is simpler as there's no direct "Assistant" or "Thread" concept like OpenAI.
      // The main interaction will happen via the ai-orchestrator.
      // We'll just log an activity here.
      await supabaseClient.from('agent_activities').insert({
        case_id: caseId,
        agent_name: 'Google Gemini',
        agent_role: 'Setup',
        activity_type: 'Gemini Analysis Initiated',
        content: `Google Gemini selected for analysis. Note: For document analysis, a RAG (Retrieval Augmented Generation) setup with a vector store is required.`,
        status: 'completed',
      });

      // Update the cases table with status
      const { error: updateCaseError } = await supabaseClient
        .from('cases')
        .update({
    ...
    return new Response(JSON.stringify({ message: 'AI analysis initiated successfully', caseId, threadId: openaiThreadId, assistantId: openaiAssistantId }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    });
